...
function UserCreate() {
	const { createUserCallback } = useUser();
	...
	return (
		<div className="bg-slate-950 text-white min-h-screen h-full flex flex-col justify-start items-center space-y-12">
			<Navbar />
			<div className="text-5xl font-bold">User Portal</div>
			<div className="flex flex-col border-slate-200 border-2 px-12 py-10 space-y-4">
				<h1 className="text-4xl text-center pb-2 font-bold">Create User</h1>
				...
				<button
					...
					onClick={() => {
						createUserCallback(name, email, +phoneNumber, affiliation);
					}}
					type="submit"
				>
					Create
